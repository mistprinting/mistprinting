...
export default function WorkPage() {
  return <><SiteHeader /><main>
    <PageHero eyebrow="Our work" title="Real projects." accent="Real print impact." description="Apparel, stickers, decals, and coordinated orders made by Mist Printing for organizations, events, and communities." />
    <section className="gallery-section">
      <div className="gallery-heading"><p className="section-label">Apparel · Stickers · Decals</p><p>Every piece shown here comes from the Mist Printing project archive.</p></div>
    ...
    <section className="video-section"><div className="section-heading"><p className="section-label">Behind the scenes</p><h2>See the process move.</h2></div><div className="video-grid">{processVideos.map((video) => <figure key={video.src}><video controls playsInline preload="none" poster={video.poster} aria-label={video.label}><source src={video.src} type="video/mp4" /></video><figcaption>{video.label}</figcaption></figure>)}</div></section>
